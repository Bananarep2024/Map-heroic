...
| **J5 — Groupes et passages** | 4 j | `Groupes.cs` (décomposition, glouton, recuit à énergie normalisée), `Passages.cs` (éligibilité par **fenêtre réalisable**, Kruskal borné, degrés cibles, localisation 2-4 arêtes, cellules réservées) | 200 seeds : 100 % tailles ∈ {3, 4}, degrés ∈ [1, 3], `Gg` connexe ; ≤ 5 % de seeds avec rejeu ; 100 % des passages ≥ 28 m. **Atteint : 100 % (40/40 graines complètes de bout en bout), 0 rejeu, degrés 1-3, largeur minimale 28,0 m, 160 ms en moyenne.** Dureté aux frontières de groupes **0,29-0,33** contre 0,21-0,25 à l'intérieur — le seuil de 0,5 de la conception n'est pas atteignable et n'a pas de sens : P5 ayant déjà posé toutes les frontières de zones sur le relief, la mesure qui compte est l'écart entre frontières retenues et frontières écartées, pas la valeur absolue |
